    ...
    $('#content_url').keyup(function() {
      ...
      $("#link_image").val("");
        ...
        success: function(data, status, xhr) {
          //if (data.images.length > 1) $('#new_content > ul.url > .' + contentType + '.tooltip .dropdown:not(.info)').show();
          if (data.title != "(no title)" || data.description != "(no text)") {
